...
static const unsigned int crc16tab[256] = { 0x0000, 0x1021, 0x2042, 0x3063,
 ...
 * @return 计算得到的校验值
 */
unsigned int Crc16(unsigned int* buf, int len)
{
    int counter;
    unsigned int crc = 0;
    for (counter = 0; counter < len; counter++)
    {
        crc = (crc << 8) ^ crc16tab[((crc >> 8) ^ (*buf & 0x00FF)) & 0x00FF];
        crc = (crc << 8) ^ crc16tab[((crc >> 8) ^ ((*buf >> 8) & 0x00FF)) & 0x00FF];
        buf++;
    }
    return crc;
}
